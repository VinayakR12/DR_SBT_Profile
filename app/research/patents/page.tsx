...
export default function PatentsPage() {
  const [content, setContent] = useState(() => normalizeResearchPatentsContent(STATIC_RESEARCH_PATENTS_CONTENT))
  ...
  useEffect(() => {
    ...
    const loadContent = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`research-patents-content API returned ${response.status}`)
        }
        ...
        if (payload.source !== 'supabase' && payload.message) {
          console.error('[research-patents-page] Falling back to static backup:', payload.message)
        }
      } catch (error) {
        if (!active) {
          return
        ...
        console.error('[research-patents-page] Failed loading DB content. Rendering static fallback.', error)
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {[
                { n: content.stats.totalPatents, l: 'Utility Patents' },
                { n: content.stats.totalCopyrights, l: 'Copyrights' },
                { n: content.stats.publishedPatents, l: 'Published' },
              ].map((s, i) => (
                <div
                  key={i}
                  style={{
                    textAlign: 'center',
                    paddingRight: i < 2 ? 'clamp(14px,3vw,36px)' : 0,
      ...
      <section style={{ background: 'var(--white)', padding: 'clamp(52px,9vh,96px) 0' }}>
        <div className="W">
          <motion.div {...up()} style={{ marginBottom: 'clamp(32px,5vh,52px)' }}>
            <p className="lbl" style={{ marginBottom: 14 }}>{content.patentsSection.kicker}</p>
            <h2 style={{ fontFamily: 'Playfair Display,serif', fontSize: 'clamp(26px,3.5vw,44px)', fontWeight: 700, color: 'var(--navy)', lineHeight: 1.1 }}>
              {content.patentsSection.titlePrefix}{' '}<em style={{ color: 'var(--gold)', fontStyle: 'italic', fontWeight: 500 }}>{content.patentsSection.titleEmphasis}</em>
            </h2>
            {!!content.patentsSection.description && <p style={{ fontSize: 15, color: 'var(--ink-3)', lineHeight: 1.75, marginTop: 14, maxWidth: 620, fontWeight: 300 }}>{content.patentsSection.description}</p>}
          </motion.div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 28 }}>
            {content.patents.map((p, i) => {
              const ss = PATENT_STATUS_STYLES[p.status] || PATENT_STATUS_STYLES.Pending
              return (
                  ...
                  >
                    ...
                    <div style={{ padding: 'clamp(24px,3.5vw,40px)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20, flexWrap: 'wrap', gap: 14 }}>
                        ...
                        {!!p.assetUrl && (
                          <button
                            onClick={() => openDoc(p.assetUrl || '', p.title)}
                      ...
                      {!!p.socialLink && (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          style={{ marginTop: 14, display: 'inline-flex', alignItems: 'center', gap: 7, padding: '9px 14px', borderRadius: 8, border: '1.5px solid rgba(45,91,138,0.26)', background: 'rgba(45,91,138,0.08)', color: '#2D5B8A', fontSize: 12.5, fontWeight: 600, textDecoration: 'none' }}
                        >
      ...
      <section style={{ background: 'var(--off)', padding: 'clamp(52px,9vh,96px) 0' }}>
        <div className="W">
          <motion.div {...up()} style={{ marginBottom: 'clamp(32px,5vh,52px)' }}>
            <p className="lbl" style={{ marginBottom: 14 }}>{content.copyrightsSection.kicker}</p>
            <h2 style={{ fontFamily: 'Playfair Display,serif', fontSize: 'clamp(26px,3.5vw,44px)', fontWeight: 700, color: 'var(--navy)', lineHeight: 1.1 }}>
              {content.copyrightsSection.titlePrefix} <em style={{ color: 'var(--gold)', fontStyle: 'italic', fontWeight: 500 }}>{content.copyrightsSection.titleEmphasis}</em>
            </h2>
            <p style={{ fontSize: 15, color: 'var(--ink-3)', lineHeight: 1.75, marginTop: 14, maxWidth: 560, fontWeight: 300 }}>{content.copyrightsSection.description}</p>
          </motion.div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 20 }}>
            {content.copyrights.map((c, i) => (
              <motion.div key={c.id} {...up(i * 0.08)}>
                ...
                >
                  ...
                  {[
                    ...
                    ['Diary No.', c.diaryNo, true],
                    ['Registered', c.date, false],
                  ].map(([k, v, mono]) => (
                  ...
                  {c.assetUrl && (
                    <button
                      onClick={() => openDoc(c.assetUrl || '', c.title)}
                  ...
                  {!!c.socialLink && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      style={{ marginTop: 10, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '9px 14px', borderRadius: 7, border: '1.5px solid rgba(45,91,138,0.26)', background: 'rgba(45,91,138,0.08)', color: '#2D5B8A', fontSize: 12.5, fontWeight: 600, textDecoration: 'none', width: '100%' }}
                    >
                      <ExternalLink size={13} /> Social Link
                    </a>
                  )}
        ...
        <div className="W" style={{ position: 'relative' }}>
          <motion.div {...up()}>
            <h2 style={{ fontFamily: 'Playfair Display,serif', fontSize: 'clamp(24px,3.5vw,40px)', fontWeight: 700, color: 'var(--navy)', lineHeight: 1.12, maxWidth: 560, margin: '0 auto 14px' }}>
              {content.cta.titlePrefix} <em style={{ color: 'var(--gold)', fontStyle: 'italic', fontWeight: 500 }}>{content.cta.titleEmphasis}</em>
            </h2>
            <p style={{ fontSize: 15, color: 'var(--ink-3)', maxWidth: 460, margin: '0 auto 32px', lineHeight: 1.75, fontWeight: 300 }}>
              {content.cta.description}
            </p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
              <Link href={content.cta.primaryHref} className="btn-navy" style={{ padding: '11px 22px', fontSize: 13.5 }}>{content.cta.primaryLabel} <ArrowRight size={14} /></Link>
              <Link href={content.cta.secondaryHref} className="btn-out" style={{ padding: '11px 22px', fontSize: 13.5 }}>{content.cta.secondaryLabel}</Link>
            </div>
          </motion.div>
        </div>
      </section>
